...
void consommateur(MemoirePartagee m, int plein, int vide, int mutex, char *chaineRecue) {
    int numeroLettre = 0;
    char c = 0;
    ...
    do {
        ...
        V(mutex);
        V(vide);
        consommerObjet(m, c, chaineRecue, numeroLettre);
...
void producteur(MemoirePartagee m, int plein, int vide, int mutex, char *chaineAEnvoyer) {
    ...
    do {
        c = produireObjet(m, chaineAEnvoyer, numeroLettre);
        P(vide);
        P(mutex);
        placerObjet(m, c);
